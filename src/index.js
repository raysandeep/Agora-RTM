...
$(() => {
    ...
    rtm.on('RemoteInvitationRefused', () => {
        const view = $('<div/>', {
            text: ['event: RemoteInvitationRefused ']
        })
        $('#log').append(view)
    })
    ...
    rtm.on('LocalInvitationAccepted', () => {
        const view = $('<div/>', {
            text: ['event: LocalInvitationAccepted ']
        })
        $('#log').append(view)
    })
    rtm.on("RemoteInvitationReceived", (RemoteInvitation) => {
        console.log('channel RemoteInvitationReceived', RemoteInvitation)
        const view = $('<div/>', {
            text: ['event: RemoteInvitationReceived from : ' + RemoteInvitation.callerId]
        })
        RemoteInvitationRequest = RemoteInvitation;
        $('#log').append(view);
        $('#log').append(`<button id="acceptpeer" >Accept Call</button>`);
        $('#log').append(`<button id="rejectpeer" >Reject Call</button>`);
    })
    rtm.on("LocalInvitationFailure", () => {
        console.log('channel LocalInvitationFailure')
        const view = $('<div/>', {
            text: ['event: LocalInvitationFailure ']
        })
        $('#log').append(view)
    })

    rtm.on("RemoteInvitationFailure", () => {
        console.log('channel RemoteInvitationFailure')
        const view = $('<div/>', {
            text: ['event: RemoteInvitationFailure ']
        })
        $('#log').append(view)
    })
    ...
    $("#acceptpeer").on('click', async function(e) {
        e.preventDefault()
        console.log("Accepting Peer");
        RemoteInvitationRequest.accept();
    })
    $("#rejectpeer").on('click', async function(e) {
            e.preventDefault()
            console.log("Accepting Peer");
            RemoteInvitationRequest.refuse();
        })
        // $("#calluser")
    $('#callpeer').on('click', function(e) {
        e.preventDefault()
        if (!rtm._logined) {
            Toast.error('Please Login First')
            return
        ...
        if (!validator(params, ['appId', 'accountName', 'memberId', 'channelName'])) {
            return
        }

        var local = rtm.createLocalInvitation(params.memberId);
        console.log("Call Intiated : ", local);
